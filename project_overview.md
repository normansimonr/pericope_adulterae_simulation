...
3.  **Execution**: The `execution/` module will manage the running of Monte Carlo simulations. This involves iteratively applying copying rules, stochastic variations (defined in `core/model.py` and utilizing `core/rng.py`), handling manuscript **migration**, and **dynamically spawning new manuscripts based on regional demand** to the textual state over a simulated historical period.
