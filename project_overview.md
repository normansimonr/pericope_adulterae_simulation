...
**Dual-Regime Execution:** Every simulation run automatically executes the text replay layer twice—once for the `"insertion"` regime and once for the `"omission"` regime—using the same demographic history. This allows for direct, controlled comparison of the two regimes on an identical population.

### Single-Run Execution

The lowest-level execution entry point is the `pasim.execution.runner.run_single` function. This function executes a single, in-memory simulation run, performing both demographic generation and dual-regime text replay, and automatically persists its results to a unique run directory.
...
-   `state` (`GenerationState`): The final demographic state of the simulation.
-   `graph` (`nx.DiGraph`): The complete genealogy graph.
-   `config` (`SimulationConfig`): The validated Pydantic configuration object used for the run.
-   `seed` (int): The demographic seed used for the run.
-   `genealogy_snapshot` (`GenealogySnapshot`): The extracted demographic record.
-   `replays` (`Dict[str, ReplayResult]`): A dictionary containing the results for both `"insertion"` and `"omission"` regimes, including the final `instance_texts` and the deterministic `replay_seed` used for each.
...
*(Note: Direct textual inspection is performed via the `replays` attribute of the `SimulationResult`.)*
...
-   **Shared Demographic Data (Run Root - `runs/run_<run_id>/`)**:
    ...
    -   `genealogy_snapshot.json`: The serialisable demographic record used for replay.
...
3.  **Execution**: The `execution/` module will manage the running of Monte Carlo simulations. This involves iteratively applying mechanistic rules (like death and migration), processing exogenous historical events, and dynamically spawning new manuscripts based on regional demand. For each new witness instance, its textual content is generated either as an initial autograph text (using `text_initialisation.py`) or by copying and mutating its exemplar(s) through the `scribal_rules` pipeline. The autograph genome is regime-dependent: it is initialized as all `0` for the `"insertion"` regime and all `1` for the `"omission"` regime. Deterministic textual transmission then follows. A specific **PA Intervention** occurs once per simulation at `pa_intervention_year` in `pa_intervention_region`. An "innovator" node is selected among those born at that tick and region, based on the highest local manuscript density (neighbor count within a fixed radius). This innovator introduces the textual change (switching to all `1`s in insertion or all `0`s in omission) and is assigned `pa_innovator_reputation`. All stochasticity is controlled via `core/rng.py` to ensure reproducibility.
